...
import { useCanvasSnap, type SnapResult, type CanvasSnapOptions } from './useCanvasSnap'
...
export const useLayerInteraction = (
  ...
  const handleMouseMove = useCallback((coordinates: CanvasCoordinates) => {
    ...
    // スナップされた位置または元の位置を使用
    const finalPosition = snapResult.snapped ? snapResult.position : rawPosition

    // ドラッグ中は高頻度でスロットリング更新
    throttledPositionChange(selectedLayerId, finalPosition)
  }, [isDragging, selectedLayerId, dragOffset, findNearestSnapPoint, throttledPositionChange, onSnapStateChange, layers])
  ...
  const handleMouseUp = useCallback((finalPosition?: CanvasCoordinates) => {
    if (isDragging && selectedLayerId && finalPosition) {
      ...
      const snapResult = findNearestSnapPoint(rawPosition, selectedLayer)
      const newPosition = snapResult.snapped ? snapResult.position : rawPosition
      
      console.log(`🎯 Layer "${selectedLayerId}" drag completed. Final position:`, newPosition)
      if (snapResult.snapped) {
        console.log(`📌 Snapped to ${snapResult.snapPoint?.name} (${snapResult.snapPoint?.id}) via ${snapResult.layerAnchor}`)
      }
  ...
  }, [isDragging, selectedLayerId, dragOffset, findNearestSnapPoint, onDragComplete, debouncedPositionChange, layers])
