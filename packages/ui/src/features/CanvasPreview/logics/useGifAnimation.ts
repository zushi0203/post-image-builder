...
const calculateOriginalFrameIndex = (
  ...
  const originalIndex = Math.floor(normalizedFrame * normalizedInfo.frameSkipRatio)
...
export const useGifAnimation = (
  ...
  // GIFレイヤーのみを抽出
  const gifLayers = layers.filter(layer => 
   ...
   * 特定レイヤーのアニメーション状態を取得
   */
  const getAnimationState = useCallback((layerId: string): GifAnimationState | null => {
  ...
  const updateAnimation = useCallback((currentTime: number) => {
    ...
    for (const layer of gifLayers) {
      ...
      if (adjustedDeltaTime >= frameInterval) {
        ...
        // 元GIFフレームインデックスを計算
        const originalFrameIndex = calculateOriginalFrameIndex(nextNormalizedFrame, state.normalizedInfo)
   ...
   * 手動でフレームを設定
   */
  const setFrame = useCallback((layerId: string, frameIndex: number) => {
