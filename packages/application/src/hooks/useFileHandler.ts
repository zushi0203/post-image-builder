    ...
    async (files: File[]) => {
      ...
      const layerPromises = validFiles.map(async (file) => {
        try {
          ...
          console.log(
            `Processing file: ${file.name}, type: ${file.type}, detected as: ${imageType}`,
          );
            ...
            try {
              ...
              const gifInfo = await parseGifFrames(file, {
                ...
                onProgress: (current, total) => {
                  console.log(
                    `GIF processing progress: ${Math.round((current / total) * 100)}%`,
                  );
                },
              });
              ...
              console.log(
                `GIF processing completed in ${performance.executionTime.toFixed(2)}ms`,
              );
              ...
              console.log(
                `GIF successfully processed: ${gifInfo.frames.length} frames, ${gifInfo.totalDuration}ms duration`,
              );
            } catch (error) {
              console.warn(
                "Failed to parse GIF frames, treating as static image:",
                error,
              );
      ...
      try {
        ...
        console.log(
          `Successfully processed ${successCount} of ${validFiles.length} files`,
        );

        if (successCount < validFiles.length) {
          // TODO: ユーザーに通知する仕組みを追加
          console.warn(
            `${validFiles.length - successCount} files failed to process`,
          );
        }
      } catch (error) {
